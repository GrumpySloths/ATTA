...
def graph(x_adv, y, x_max, x_min, grad):
    ...
    with tf.variable_scope('adv1', reuse=tf.AUTO_REUSE):
        with slim.arg_scope(scope):
            logits, end_points = model(
                input_diversity(x_adv), num_classes=num_classes, is_training=False)
    with slim.arg_scope(scope):
        logits_adv, end_points_adv = model(
            input_diversity(x_trans), num_classes=num_classes, is_training=False,reuse=tf.AUTO_REUSE)
    ...
    # x_adv_4=0.25*x_adv
    # x_trans_4=0.25*x_trans
    # with tf.variable_scope('adv1', reuse=tf.AUTO_REUSE):
    #     with slim.arg_scope(scope):
    #         logits_4, end_points = model(
    #             input_diversity(x_adv_4), num_classes=num_classes, is_training=False)
    # with slim.arg_scope(scope):
    #     logits_adv_4, end_points_adv = model(
    #         input_diversity(x_trans_4), num_classes=num_classes, is_training=False,reuse=tf.AUTO_REUSE)
    # loss_adv_4 = tf.reduce_mean(tf.losses.softmax_cross_entropy(one_hot, logits_adv_4))
    # loss_4 = tf.reduce_mean(tf.losses.softmax_cross_entropy(one_hot, logits_4))
    # loss_fool_4=loss_4+FLAGS.gamma*loss_adv_4
    # noise += optimizer.compute_gradients(loss_fool_4, var_list=[x_adv_4])[0][0]

    # x_adv_8=1/8*x_adv
    # x_trans_8=1/8*x_trans
    # with tf.variable_scope('adv1', reuse=tf.AUTO_REUSE):
    #     with slim.arg_scope(scope):
    #         logits_8, end_points = model(
    #             input_diversity(x_adv_8), num_classes=num_classes, is_training=False)
    # with slim.arg_scope(scope):
    #     logits_adv_8, end_points_adv = model(
    #         input_diversity(x_trans_8), num_classes=num_classes, is_training=False,reuse=tf.AUTO_REUSE)
    # loss_adv_8 = tf.reduce_mean(tf.losses.softmax_cross_entropy(one_hot, logits_adv_8))
    # loss_8 = tf.reduce_mean(tf.losses.softmax_cross_entropy(one_hot, logits_8))
    # loss_fool_8=loss_8+FLAGS.gamma*loss_adv_8
    # noise += optimizer.compute_gradients(loss_fool_8, var_list=[x_adv_8])[0][0]

    # x_adv_16=1/16*x_adv
    # x_trans_16=1/16*x_trans
    # with tf.variable_scope('adv1', reuse=tf.AUTO_REUSE):
    #     with slim.arg_scope(scope):
    #         logits_16, end_points = model(
    #             input_diversity(x_adv_16), num_classes=num_classes, is_training=False)
    # with slim.arg_scope(scope):
    #     logits_adv_16, end_points_adv = model(
    #         input_diversity(x_trans_16), num_classes=num_classes, is_training=False,reuse=tf.AUTO_REUSE)
    # loss_adv_16 = tf.reduce_mean(tf.losses.softmax_cross_entropy(one_hot, logits_adv_16))
    # loss_16 = tf.reduce_mean(tf.losses.softmax_cross_entropy(one_hot, logits_16))
    # loss_fool_16=loss_16+FLAGS.gamma*loss_adv_16
    # noise += optimizer.compute_gradients(loss_fool_16, var_list=[x_adv_16])[0][0]
