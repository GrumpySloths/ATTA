...
tf.flags.DEFINE_string('input_dir', './outputs/di_fgsm_inceptionv3',
                                    'Input directory with images.')

tf.flags.DEFINE_string(
    'output_file', './outputs/nipsr3.txt', 'Output file to save labels.')
...
class EnsModel(object):
  ...
  def __call__(self, ens_x_input, vgg_x_input, inc_x_input, tcd_x_input):
    ...
    reuse_flags = [tf.AUTO_REUSE, True]
    ...
    with slim.arg_scope(resnet_utils.resnet_arg_scope()):
      weight = 0.05
      result = resnet_v2.resnet_v2_101(vgg_x_input, num_classes=self.num_classes, reuse=reuse)
...
def main(_):
  ...
  with graph.as_default():
    ...
    res_saver = tf.train.Saver(slim.get_model_variables('resnet_v2'))
  ...
  res_saver.restore(sess, model_checkpoint_map['resnet_v2'])
  vgg_saver.restore(sess, model_checkpoint_map['vgg_16'])
  inc_res_saver.restore(sess, model_checkpoint_map['InceptionResnetV2'])
  adv_inc_saver.restore(sess, model_checkpoint_map['InceptionV3'])
